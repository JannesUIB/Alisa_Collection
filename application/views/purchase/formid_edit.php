...
<html lang="en">
		...
		<div class="container shadow p-3 mb-3 mt-5 bg-white rounded">
			<div class="row">
				<div class="col">
					...
					?>
					<div class="">
						<form id="purchase_form" method="POST" action="<?php echo site_url('Purchase/UpdatePurchase/' . $rec->ID); ?>">
							<div class="form-group">
								<input type="text" class="form-control" placeholder="Purchase Order ID" value="<?php echo $rec->Purchase_Name?>" style="border-top:0px solid black;border-right:0px solid black;border-left:0px solid black;height:80px;font-size:24px; width:50%" readonly>
							</div>
							<div class="form-group">
							...
							<div class="row" style="margin-top:10px;"> 
								<div class="col">
									<table id="table_pol" class="table table-sm table-bordered border-dark">
										<thead>
											<tr>
												<th class="d-none" id="POL_ID">POL ID</th>
										...
										<tbody>
												<?php 
													foreach($purchase_order_line_record as $reco){
													echo "<tr>";
							...
							<button type="button" onclick="submitForm()" class="btn btn-info btn-lg btn-block" style="margin-top:10px;">Update Purchase</button>
						</form>
					</div>
				</div>
			</div>
	...
	<script>
		function submitForm() {
			...
			var table = document.getElementById("table_pol");
			...
			document.getElementById("purchase_form").appendChild(hiddenInput);
		...
		document.addEventListener("DOMContentLoaded", function() {
			...
			document.getElementById("insert_item").addEventListener("click", function() {
				...
				document.getElementById("table_pol").getElementsByTagName('tbody')[0].appendChild(newRow);
